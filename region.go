...
func (r *Region) Each(clb func(chunk *Chunk)) {
	...
	for cx := 0; cx < 32; cx++ {
		for cz := 0; cz < 32; cz++ {
			chunk := getChunkFromReadSeeker(b, cx, cz)
			if chunk == nil {
				continue
			}
			clb(chunk)
...
func processRMCAFile(p string, chunkX, chunkZ int) *Chunk {
	f, err := os.Open(p)
	if err != nil {
		log.Println(err)
		return nil
	...
	return getChunkFromReadSeeker(f, chunkX, chunkZ)
}

func getChunkFromReadSeeker(f io.ReadSeeker, chunkX, chunkZ int) *Chunk {
